...
Investigation Steps & Validation
    ...
    Telemetry Generation: Executed a monitored PowerShell process invocation forcing command-line parameter logging (Get-ComputerInfo).

    Index / Query Tuning: Verified that the query language setting in Discover matched the syntax being used (switching correctly between KQL and ES|QL modes).

    Result Validation: Confirmed event capture via Elastic Agent and successfully isolated the process execution parameters in the SIEM dashboard.
